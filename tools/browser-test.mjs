...
const scratch=fs.mkdtempSync(path.join(os.tmpdir(),'stellar-continuous-')),ids=['v2-ch01','v2-ch02','v2-ch03'];
...
try{
 ...
 assert.deepEqual(errors,[]);assert.deepEqual(outbound,[]);console.log(JSON.stringify({status:'passed',chapters:3,questions:19,activities:8,wrongAnswerRetry:'all-19',keyboard:'sliders-card-and-quiz',nativeSelectKeyboard:'not-verified-in-headless-macOS',zoom:'200-percent-CSS-zoom',feedback:JSON.parse(fs.readFileSync(path.join(ROOT,'tools/feedback.json'),'utf8')).formUrl?'link-configured-submission-not-tested':'pending-feishu-authorization',screenshots:scratch,errors,outbound},null,2));
