...
def main():
 out=['# 四书目录与本地阅读入口','', '本文件是导航，不是正文核实报告。小节标题来自 MinerU 正文及目录，可能含 OCR 误差。', '',
 ...
 for book,manifest,chapters,aux in books():
  original=ROOT/'library'/book/manifest['原始PDF']
  out += [f'## {book}（{len(chapters)} 个正文编号章节）','',link(original,'整卷 PDF')+' · '+link(ROOT/'library'/book/'按书签拆分/拆分清单.json','拆分清单'),'',f'原始 PDF SHA-256：`{hashlib.sha256(original.read_bytes()).hexdigest()}`','']
  for c in chapters:
  ...
  supplemental=ROOT/'library'/book/'supplemental_sources'
